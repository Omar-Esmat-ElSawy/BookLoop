...
const EditBookPage = () => {
  ...
  useEffect(() => {
    const loadBook = async () => {
      ...
      try {
        ...
        if (!bookData) {
          toast.error(t('toasts.bookNotFound'));
          navigate('/my-books');
          return;
        }
        ...
        if (user && bookData.owner_id !== user.id) {
          toast.error(t('toasts.noPermissionToEditBook'));
          navigate('/books');
          return;
        }
  ...
  const handleUpdateBook = async (
    ...
    try {
      ...
      if (updatedBook) {
        toast.success(t('toasts.bookUpdatedSuccess'));
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="flex items-center gap-3 p-3 rounded-lg border bg-card">
              <Label htmlFor="availability" className="text-sm font-medium">
                {book.is_available ? t('books.availableForExchange') : t('books.notAvailable')}
              </Label>
              <Switch
                id="availability"
