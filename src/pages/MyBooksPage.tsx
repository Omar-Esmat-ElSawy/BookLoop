  ...
  const isMobile = useIsMobile();
   ...
   const getTranslatedStatus = (status: string) => {
     const statusMap: Record<string, string> = {
       'pending': t('exchange.pending'),
       'accepted': t('exchange.accepted'),
       'rejected': t('exchange.rejected'),
       'cancelled': t('exchange.cancelled'),
       'completed': t('exchange.completed'),
       'done': t('exchange.done'),
     };
      ...
      <main className="container mx-auto px-4 py-6 md:py-8 flex-grow">
        ...
        <Tabs defaultValue="my-books" className="w-full">
          <TabsList className="mb-6 w-full md:w-auto flex">
             <TabsTrigger value="my-books" className="flex-1 md:flex-none">{t('myBooks.myBooksTab')}</TabsTrigger>
          ...
          <TabsContent value="my-books">
            {loading ? (
              ...
              userBooks.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-64">
                   <p className="text-muted-foreground">{t('myBooks.noBooksYet')}</p>
                </div>
              )
            )}
          </TabsContent>
          ...
          <TabsContent value="incoming-requests">
            {loadingRequests ? (
                  ...
                  <div key={request.id} className="border rounded-lg p-3 md:p-4 flex flex-col md:flex-row gap-3 md:gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-medium text-sm md:text-base">{request.book?.title || 'Untitled Book'}</h3>
                       <p className="text-xs md:text-sm text-muted-foreground mb-2">{t('myBooks.status')}: {getTranslatedStatus(request.status)}</p>
                      {request.message && (
                      ...
                      <div className="flex gap-2">
                        <Link to={`/books/${request.book_id}`}>
                           <Button variant="outline" size={isMobile ? "sm" : "default"}>{t('myBooks.viewBook')}</Button>
                        </Link>
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-center h-64">
                 <p className="text-muted-foreground">{t('myBooks.noIncomingRequests')}</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="outgoing-requests">
            {loadingRequests ? (
                  ...
                  <div key={request.id} className="border rounded-lg p-3 md:p-4 flex flex-col md:flex-row gap-3 md:gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs md:text-sm text-muted-foreground mb-2">
                         {t('myBooks.status')}: <span className={
                          request.status === 'accepted' ? 'text-green-600' :
                          request.status === 'rejected' ? 'text-destructive' :
                          ''
                         }>{getTranslatedStatus(request.status)}</span>
                      </p>
                      {request.message && (
                        <p className="text-xs md:text-sm border-l-2 pl-3 py-1 mb-2 md:mb-3 italic line-clamp-2 md:line-clamp-none">"{request.message}"</p>
                      )}
              ...
              <div className="flex items-center justify-center h-64">
                 <p className="text-muted-foreground">{t('myBooks.noOutgoingRequests')}</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
