...
const BookForm = ({ book, genres, onSubmit, isSubmitting }: BookFormProps) => {
  ...
  const getLocalizedGenreName = (genre: BookGenre) => {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-1">
            <CardHeader>
              <CardTitle>{t('form.coverImage')}</CardTitle>
              <CardDescription>
                {t('form.coverImageDesc')}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-col items-center gap-4">
                ...
                >
                  {imagePreview ? (
                    ...
                    <span className="text-muted-foreground">{t('form.noImage')}</span>
                  )}
                </div>
                <Label htmlFor="cover-image" className="cursor-pointer w-full">
                  <div className="flex items-center justify-center w-full px-4 py-2 border border-input rounded-md text-sm hover:bg-secondary transition-colors">
                    {book ? t('form.changeImage') : t('form.uploadImage')}
                  </div>
                  <Input 
                    id="cover-image" 
                    type="file" 
          ...
          <Card className="md:col-span-2">
            <CardHeader>
              <CardTitle>{t('form.bookDetails')}</CardTitle>
              <CardDescription>
                {t('form.bookDetailsDesc')}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder={t('form.enterBookTitle')} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder={t('form.enterAuthorName')} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea 
                        placeholder={t('form.enterBookDescription')} 
                        rows={4} 
                        {...field} 
                      />
                    </FormControl>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('form.genre')}</FormLabel>
                      <Select 
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue placeholder={t('form.selectGenre')} />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          {genres.map((genre) => (
            ...
            <CardFooter>
              <Button type="submit" className="w-full" disabled={isSubmitting}>
                {isSubmitting ? t('common.submitting') : book ? t('form.updateBook') : t('books.addBook')}
              </Button>
            </CardFooter>
          </Card>
        </div>
