...
// Genre translation map (English genre name to translation key)
...
const BookCard = ({ book, className }: BookCardProps) => {
  ...
  const getTranslatedGenre = (genre: string | null): string => {
    if (!genre) return '';
    const lowerGenre = genre.toLowerCase();
    const translationKey = genreKeyMap[lowerGenre];
    if (translationKey) {
      return t(translationKey);
    }
    return genre; // Return original if no translation found
  };
      ...
      <div className="relative aspect-[2/3] overflow-hidden">
        ...
        {!book.is_available && (
          <div className="absolute inset-0 bg-background/80 flex items-center justify-center backdrop-blur-sm">
            <Badge variant="secondary" className="text-xs px-2 py-0.5 md:text-sm md:px-3 md:py-1">{t('books.notAvailable')}</Badge>
          </div>
        )}
      </div>
...
const getGenreColor = (genre: string | null): string => {
