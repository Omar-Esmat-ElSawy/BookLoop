    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          toast.success(t('toasts.coordinatesSavedEnterCity'));
        } finally {
      ...
      (error) => {
        ...
        switch (error.code) {
          ...
          case error.TIMEOUT:
            ...
            break;
          default:
            toast.error(t('toasts.failedToGetLocation'));
        }
      }
    );
  };
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-2">
        <Label htmlFor="city">{t('profile.CityLocation')}</Label>
        ...
        {latitude && longitude && (
          <p className="text-xs text-muted-foreground">
            {t('profile.Coordinates')}: {latitude.toFixed(4)}, {longitude.toFixed(4)}
          </p>
        )}
        <p className="text-xs text-muted-foreground">
          {t('profile.LocationHelpText')}
        </p>
      </div>
    </div>
  );
