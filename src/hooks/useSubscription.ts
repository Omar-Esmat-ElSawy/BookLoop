...
export const useSubscription = () => {
  ...
  const createCheckoutSession = useCallback(async () => {
    if (!user) {
      toast.error(t("toasts.pleaseLoginToSubscribe"));
      return;
    }
    ...
    try {
      ...
      toast.error(t("toasts.failedToCreateCheckout"));
    }
  }, [user]);

  const openCustomerPortal = useCallback(async () => {
    if (!user) {
      toast.error(t("toasts.pleaseLoginToManageSubscription"));
      return;
    }
    ...
    try {
      ...
      toast.error(t("toasts.failedToOpenCustomerPortal"));
    }
  }, [user]);
