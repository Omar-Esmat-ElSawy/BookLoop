...
export const BooksProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addBook = async (
    ...
    try {
      ...
      toast.success(t('toasts.bookAddedSuccess'));
      return data as Book;
    } catch (error: any) {
  ...
  const deleteBook = async (id: string): Promise<void> => {
    try {
      ...
      toast.success(t('toasts.bookDeletedSuccess'));
    } catch (error: any) {
  ...
  const requestBookExchange = async (bookId: string, message: string, offeredBookId?: string): Promise<boolean> => {
    try {
      ...
      if (existingRequests && existingRequests.length > 0) {
        toast.error(t('toasts.alreadyRequested'));
        return false;
      }
      ...
      toast.success(t('toasts.exchangeRequestSent'));
      return true;
    } catch (error: any) {
  ...
  const cancelExchangeRequest = async (requestId: string): Promise<boolean> => {
    try {
      ...
      toast.success(t('toasts.exchangeRequestCancelled'));
      return true;
    } catch (error: any) {
  ...
  const markExchangeAsDone = async (requestId: string): Promise<boolean> => {
    try {
      ...
      toast.success(t('toasts.exchangeMarkedDone'));
      return true;
    } catch (error: any) {
  ...
  const respondToExchangeRequest = async (requestId: string, accept: boolean): Promise<boolean> => {
    try {
      ...
      toast.success(t(`toasts.exchangeRequest${accept ? 'Accepted' : 'Rejected'}Successfully`));
      return true;
    } catch (error: any) {
  ...
  const toggleBookAvailability = async (bookId: string, isAvailable: boolean): Promise<boolean> => {
    try {
      ...
      toast.success(t(isAvailable ? 'toasts.bookMarkedAvailable' : 'toasts.bookMarkedUnavailable'));
      return true;
    } catch (error: any) {
